...
static uint16_t vecDecodeTable[256][8] = {
...
void populate_positions_neon(uint64_t input, uint32_t *out, int *count) {
    uint32_t *initout = out;
    uint16x8_t baseVec = vdupq_n_u16(-1);
    uint16x8_t add8 = vdupq_n_u16(8);

    if (input != 0) {
        for (int k = 0; k < 4; ++k) {
            uint8_t byteA = (uint8_t) input;
            uint8_t byteB = (uint8_t) (input >> 8);
            input >>= 16;
            uint16x8_t vecA = vld1q_u16(vecDecodeTable[byteA]);
            uint16x8_t vecB = vld1q_u16(vecDecodeTable[byteB]);
            uint8_t advanceA = lengthTable[byteA];
            uint8_t advanceB = lengthTable[byteB];
            vecA = vaddq_u16(baseVec, vecA);
            baseVec = vaddq_u16(baseVec, add8);
            vecB = vaddq_u16(baseVec, vecB);
            baseVec = vaddq_u16(baseVec, add8);

            uint32x4_t tmpVec = vmovl_u16(vget_low_u16(vecA));
            *((uint32x4_t *) out) = tmpVec;
            if (advanceA > 4) {
                tmpVec = vmovl_u16(vget_high_u16(vecA));
                *((uint32x4_t *) (out + 4)) = tmpVec;
            }
            out += advanceA;
            tmpVec = vmovl_u16(vget_low_u16(vecB));
            *((uint32x4_t *) out) = tmpVec;
            if (advanceB > 4) {
                tmpVec = vmovl_u16(vget_high_u16(vecB));
                *((uint32x4_t *) (out + 4)) = tmpVec;
            }
            out += advanceB;
        }
